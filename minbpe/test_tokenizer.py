...
test_strings = [
    "",  # empty string
    "?",  # single character
    "hello world!!!? கேள்வி முயல் 😉",  # fun small string
]


def unpack(text_file):
    # we do this because `pytest -v .` prints the arguments to console, and we don't
    # want to print the entire contents of the file, it creates a mess. So here we go.
    if text_file.startswith("FILE:"):
        dirname = os.path.dirname(os.path.abspath(__file__))
        taylorswift_file = os.path.join(dirname, text_file[5:])
...
rt = RegexTokenizer()
rt.load(r"/home/muthu/Desktop/TamilTokenizer/regex.model")
print(rt.encode("hello world!!!? கேள்வி முயல் 😉"))
...
def test_save_load(special_tokens, tokenizer_factory):
